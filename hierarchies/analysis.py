from hierarchies.utils import set_np_printoptions, Colors
from hierarchies.graph import wnid_to_synset, synset_to_wnid
from hierarchies.model import (
    SoftEmbeddedDecisionRules as SoftRules,
    HardEmbeddedDecisionRules as HardRules,
)
from hierarchies.thirdparty.nx import read_graph
from torch.distributions import Categorical
import torch.nn.functional as F
from collections import defaultdict
import torch
from hierarchies import metrics
import functools
import numpy as np
import os
...
class Noop:
    ...
    def update_batch(self, outputs, targets, images, prototypes=None, img_dirs=None, img_idx=None, **kwargs):
        out = self._update_batch(outputs, targets, prototypes, img_dirs, **kwargs)
        return out

    def _update_batch(self, outputs, targets, prototypes, img_dirs=None, **kwargs):
        pass
...
class ConfusionMatrix(Noop):
    ...
    def _update_batch(self, outputs, targets, prototypes=None, **kwargs):
...
class IgnoredSamples(Noop):
    ...
    def _update_batch(self, outputs, targets, prototypes=None):
        super()._update_batch(outputs, targets)
...
class DecisionRules(Noop):
    ...
    def __init__(self, *args, Rules=SoftRules, tree=None, metric="top1", **kwargs):
    ...
    def _update_batch(self, outputs, targets, prototypes=None, img_dirs=None, images=None, **kwargs):
        super()._update_batch(outputs, targets, prototypes)
        if 'T' in kwargs and 'c' in kwargs:
            outputs = self.rules.forward(outputs, prototypes, T=kwargs['T'], c=kwargs['c'])
        else:
            outputs = self.rules.forward(outputs, prototypes)

        self.metric.forward(outputs, targets)

        return round(self.metric.report() * 100, 3)
...
class ScoreSave(Noop):
    ...
    def __init__(
        ...
        *args,
        classes=(),
        save_k=50,
        path="out_score/{suffix}-{i}-{image_name}-{score:.2e}.jpg",
        **kwargs,
    ):
    ...
    def update_batch(self, outputs, targets, images, prototypes=None, img_dirs=None, img_idx=None, **kwargs):
        super().update_batch(outputs, targets, images, prototypes, img_dirs, img_idx)
        scores = self.score(outputs, targets, images)
        ois = list(zip(outputs, images, scores, img_dirs, img_idx))
    ...
    def end_test(self, epoch):
        ...
        for name, suffix, lst in (("highest", "max", self.max), ("lowest", "min", self.min),):
            ...
            for i, (_, image, score, img_dir, img_idx) in enumerate(lst):
                Image.fromarray(
...
class Entropy(ScoreSave):
    ...
    def __init__(
        self,
        *args,
        path="out_entropy/{suffix}-{i}-{idx}-{image_name}-{score:.2e}.jpg",
        **kwargs,
    ):
    ...
    def update_batch(self, outputs, targets, images, prototypes=None, img_dirs=None, img_idx=None, **kwargs):
        super().update_batch(outputs, targets, images, prototypes, img_dirs, img_idx, **kwargs)
    ...
    def end_test(self, epoch):
        super().end_test(epoch)
        print(
            f"[Entropy] avg {self.avg:.2e}, std {self.std:.2e}, max {float(self.max[0][-3]):.2e}, min {float(self.min[0][-3]):.2e}"
        )
...
class NBDTEntropyMaxMin(Entropy):
    ...
    def score(self, outputs, targets, images, prototypes=None):
        decisions = self.rules.forward_with_decisions(outputs, prototypes)
...
class VisualizeHierarchyInference(SoftEmbeddedDecisionRules):
    ...
    def update_batch(self, outputs, targets, images, prototypes=None, **kwargs):
        tree = self.rules.tree
        if prototypes is not None:
            wnid_to_outputs = self.rules.forward_nodes(outputs, prototypes)
        else:
            wnid_to_outputs = self.rules.forward_nodes(outputs)

        #outputs = self.rules.forward(outputs, wnid_to_outputs)
        outputs = self.rules.traverse_tree(wnid_to_outputs, tree)
        preds = torch.argmax(outputs, dim=1)

        for j in range(len(targets)):
            G = read_graph(tree.path_graph)
            ...
            for node in tree.nodes:
                if not node.parent or node.parent.wnid not in wnid_to_outputs:
                    ...
                    continue
                probs = wnid_to_outputs[node.parent.wnid]['probs']     # get the parent outputs
                child_index = node.parent.wnid_to_child_index(node.wnid)   # get the index

                if probs[j, child_index].item() < 0.05 and node.is_leaf():
                    remove_nodes.append(node.wnid)
                else:
