...
def add_arguments(parser):
    parser.add_argument(
        "--focal-gamma", "--fg", type=float, default=0, help="gamma for focal loss"
    )
    parser.add_argument(
        "--xent-weight", "--xw", type=float, default=1, help="Weight for cross entropy term"
    )
    parser.add_argument(
        "--xent-weight-end",
        "--xwe",
    ...
    parser.add_argument(
        "--xent-weight-power", "--xwp", type=float, default=1, help="Raise progress to this power."
    )
    parser.add_argument(
...
class TreeSupLoss(nn.Module):
    ...
    accepts_dataset = True  # lambda trainset, **kwargs: trainset.__class__.__name__
    accepts_path_graph = True
    accepts_path_wnids = True
    accepts_tree_supervision_weight = True
    accepts_classes = True  # lambda trainset, **kwargs: trainset.classes
    ...
    def forward_tree(self, outputs, targets, features=None, prototypes=None):
    ...
    def forward(self, outputs, targets, features=None, prototypes=None):
        loss_xent = self.criterion(outputs, targets)
        loss_tree = self.forward_tree(outputs, targets, features, prototypes)
    ...
    def forward_tree(self, outputs, targets, features=None, prototypes=None):
        ...
        if features is not None and prototypes is not None:
            if self.focal_gamma != 0:
                pt = self.rules(features, prototypes)
                tree_ce_loss = nn.NLLLoss(reduction="none")(torch.log(pt + 1e-20), targets)   # batch
                loss = ((1 - pt.gather(dim=-1, index=targets.unsqueeze(1)).view(-1)) ** self.focal_gamma * tree_ce_loss).mean()
                # print('focal loss ', loss.item(), 'loss ', nn.NLLLoss()(torch.log(self.rules(features, prototypes) + 1e-20), targets).item())
                return loss
            else:
                return nn.NLLLoss()(torch.log(self.rules(features, prototypes) + 1e-20), targets)
            #SSreturn self.criterion(self.rules(features, prototypes), targets)
        # todo: check if use nll loss is better than directly apply cross-entropy on path probs
        else:
            return nn.NLLLoss()(torch.log(self.rules(outputs)+1e-20), targets)
...
class SoftTreeLoss(SoftTreeSupLoss):
    ...
    def forward_tree(self, outputs, targets, prototypes=None):
...
def get_weight(start, end, progress, power=1.2):
...
#     progress = cur / total
#     w.append(get_weight(0, 1, progress,0.4))
# print(w)
#
# plt.plot(w)